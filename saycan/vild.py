...
class ViLD:
    def __init__(self, clip_model):
        ...
        print("Input resolution:", self.clip_model.visual.input_resolution)
        print("Context length:", self.clip_model.context_length)
        print("Vocab size:", self.clip_model.vocab_size)
        ...
        self.saved_model_dir = "./image_path_v2"
        _ = tf.saved_model.loader.load(self.session, ["serve"], self.saved_model_dir)
    ...
    def inference(self, image_path, category_name_string, params, plot_on=True, prompt_swaps=[]):
        #################################################################
        ...
        if len(indices_fg) == 0:
            self.display_image(np.array(image), size=self.overall_fig_size, image_path=image_path)
    ...
    def display_image(self, path_or_array, size=(10, 10), image_path=None):
    ...
    def paste_instance_masks(self, masks,
                            ...
                            image_height,
                            image_width):
        """Paste instance masks to generate the image segmentation r代码esults.
        ...
        # Reference: https://github.com/facebookresearch/Detectron/b代码lob/master/detectron/core/test.py#L812  # pylint: disable=line-too-long
...
if __name__ == "__main__":
    ...
    vild_model = ViLD(clip_model)
    found_objects = vild_model.inference(image_path, category_name_string, vild_params, plot_on=True, prompt_swaps=prompt_swaps)